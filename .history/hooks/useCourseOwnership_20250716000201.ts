...
export function useAllCourseOwnership() {
  ...
  // Get all course ownership for the user using purchasedCourses array
  const loadAllOwnership = useCallback(async () => {
    ...
    try {
      // First try to get ownership from purchasedCourses array (faster and more reliable)
      const userDoc = await userPurchasesService.getUserDocument(user?.id);
      const purchasedCourses = userDoc?.purchasedCourses || [];

      console.log("User purchased courses from array:", purchasedCourses);

      // Convert to CourseOwnership format
      const ownershipData = purchasedCourses.map((courseId: string) => ({
        courseId,
        isOwned: true,

...
export const blogService = {
  ...
  async getAllBlogPosts(): Promise<BlogPost[]> {
    const blogData: any = await getDocuments(COLLECTION_NAME);
    return blogData || [];
  },

  // Get a single blog post by ID
  async getBlogPostById(id: string): Promise<BlogPost | null> {
    const blogData = await getDocument(COLLECTION_NAME, id);
  ...
  async getBlogPostByUrl(url: string): Promise<BlogPost | null> {
    const blogData = await getDocument(COLLECTION_NAME, url);
    if (!blogData?.posts) return null;
  ...
  async addBlogPost(
    ...
    await updateDocument(COLLECTION_NAME, "blog", {

...
export default function NewBlogPostPage() {
  ...
  const moveSection = (index: number, direction: "up" | "down") => {
    ...
    for (let i = 0; i < newSections.length; i++) {
      if (i === index) {
        // Skip the current section, it will be inserted at targetIndex
        continue;
      } else if (i === targetIndex) {
        // Insert the moved section at targetIndex
        reorderedSections.push(newSections[index]);
        reorderedSections.push(newSections[i]);
      } else {
    ...
    // Handle edge case where targetIndex is at the end
    if (
      targetIndex === newSections.length - 1 &&
